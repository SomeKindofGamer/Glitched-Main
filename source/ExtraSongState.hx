...
class ExtraSongState extends MusicBeatState
{
    ...
    var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('extraBackground'));
    var curSelected:Int = 0;

    private var iconArray:Array<HealthIcon> = [];

    var swagText:FlxText = new FlxText(0, 0, 0, 'swagText', 85);

    var songColors:Array<FlxColor> = [ // stolen HAHAHA
    	0xFFca1f6f, // GF
		0xFF4965FF, // DAVE
    ...
    override function create() 
	{
        ...
        bg.loadGraphic(Paths.image('extraBackground'));
		bg.color = 0xFF4965FF;
		add(bg);
        
		addWeek(['Formidity'], 5, ['bf-fusion']);
    ...
    public function addWeek(songs:Array<String>, weekNum:Int, ?songCharacters:Array<String>)
	{
		...
		for (song in songs)
		{
            if ((song.toLowerCase() == 'formidity' && !FlxG.save.data.formidityUnlocked))
                addSong('unknown', weekNum, songCharacters[num]);
            else
